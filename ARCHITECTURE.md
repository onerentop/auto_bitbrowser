...
- **标签（tag）同样存在于 ixBrowser，不自建本地字段。** 读：窗口 `tag_id`（空格分隔的多个 id）+ `tag-list` 词表映射成名字与颜色，**绝不按空格解析 `tag_name`**（标签标题本身可能含空格，真机实测有 `Google Cloud不可用`）；写：`profile-update` 的 `tag` 字段、值是**标签名数组**。只有用户在账号页显式增删标签、或在「标签管理」里改词表时才写入，自动化任务不写标签。列可见性等纯界面偏好记在 localStorage，不进 `config.json`。
