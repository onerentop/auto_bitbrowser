...
import { AI_TASKS_INVOKE, AI_TASK_KINDS, AI_TASK_STATUS_FILTERS } from "../app/shared/channels/ai-tasks.ts";
...
import { buildAiTaskTree, describeOutcome } from "../src/application/ai-task-runner.ts";
...
test("AI_TASK_KINDS：任务名 / 额外输入文案逐字照搬 Python 子类", () => {
  ...
  assert.deepEqual(
    AI_TASK_STATUS_FILTERS.map((o) => o.label),
    [
      "全部",
      "pending - 待处理",
      "link_ready - 链接就绪",
...
test("分派：replace_phone 透传 newPhone（去首尾空白），closeAfter:false", async () => {
  const s = setup();
  try {
    await s.call(START, "replace_phone", [{ email: "a@x.com", profileId: 11 }], { newPhone: " 13800 " }, 1);
    await s.finished();
...
test("分派：replace_email 透传 newEmail；未传参数时为空串", async () => {
  const s = setup();
  try {
    await s.call(START, "replace_email", [{ email: "a@x.com", profileId: 5 }], { newEmail: "r@y.com" }, 1);
    await s.finished();
    ...
    await s.call(START, "replace_email", [{ email: "b@x.com", profileId: 6 }], {}, 1);
    await s.finished();
...
test("分派：modify_2sv 不显式传 closeAfter（取函数默认值 true）", async () => {
  const s = setup();
  try {
    await s.call(START, "modify_2sv", [{ email: "a@x.com", profileId: 7 }], { newPhone: "+100" }, 1);
    await s.finished();
...
test("分派：modify_auth 注入 accountRepo / historyRepo / ixClient / projectRoot=dataRoot，并建好历史表", async () => {
  const s = setup();
  try {
    await s.call(START, "modify_auth", [{ email: "a@x.com", profileId: 8 }], {}, 1);
    await s.finished();
...
test("分派：kick_devices 只传 browserId 与 accountInfo", async () => {
  ...
  try {
    ...
    // 并发数只记录不使用
    assert.ok(s.logs().some((m) => m.includes("并发数: 3")));
  } finally {
    s.cleanup();
  }
});
...
test("load：窗口名匹配账号状态，未匹配为 pending；未分组 / 未知分组；按 gid 排序；只列有窗口的分组", async () => {
  const s = setup({
    groups: [
      { id: 2, title: "组A" },
      { id: 3, title: "\u0007" }, // 清洗后为空 → 分组 3（但没有窗口，不显示）
      { id: 5, title: "" },
    ],
    windows: [
      { profile_id: 11, name: "a@x.com", group_id: 2 },
      { profile_id: 12, name: "b@x.com" }, // 无 group_id → 未分组
      { profile_id: 13, name: "c@x.com", group_id: 9 }, // group-list 里没有 → 分组 9
  ...
  try {
    ...
    assert.deepEqual(
      res.groups.map((g) => [g.groupId, g.groupName, g.browsers.length]),
      [
        [0, "未分组", 1],
        [2, "组A", 1],
        [5, "分组 5", 1],
    ...
    const a = res.groups[1].browsers[0];
    assert.deepEqual(a, { key: "b:11", profileId: 11, name: "a@x.com", status: "verified", matched: true });
    const b = res.groups[0].browsers[0];
...
test("load：读取失败时返回 error 字段而不抛异常", async () => {
  ...
  try {
    const res = await s.call(LOAD);
    assert.deepEqual(res, { groups: [], totalBrowsers: 0, error: "ixBrowser 客户端创建失败" });
  } finally {
    s.cleanup();
  }
});
...
test("buildAiTaskTree：非法窗口 ID 为 null，重复 ID 退回序号 key", () => {
  const t = buildAiTaskTree([], [], [
    { profile_id: "x", name: "a" },
    { profile_id: 1, name: "b" },
    { profile_id: 1, name: "c" },
  ]);
...
test("参数校验：各类非法输入 → INVALID_ARGUMENT", async () => {
  ...
  const bad = [
    [],
    ["replace_phone", ok, {}], // 缺并发数
    ["nope", ok, {}, 1],
    ["replace_phone", [], {}, 1],
    ["replace_phone", "x", {}, 1],
    ["replace_phone", [null], {}, 1],
    ["replace_phone", [{ email: "", profileId: 1 }], {}, 1],
    ["replace_phone", [{ email: "a@x.com", profileId: 0 }], {}, 1],
    ["replace_phone", [{ email: "a@x.com", profileId: 1.5 }], {}, 1],
    ["replace_phone", [{ email: "a@x.com", profileId: "1" }], {}, 1],
    ["replace_phone", ok, null, 1],
    ["replace_phone", ok, [], 1],
    ["replace_phone", ok, { newEmail: "x" }, 1], // 不属于该 kind 的键
    ["modify_auth", ok, { newPhone: "1" }, 1], // 无额外输入的 kind
    ["replace_phone", ok, { newPhone: 123 }, 1],
    ["replace_phone", ok, { newPhone: "1".repeat(201) }, 1],
    ["replace_phone", ok, {}, 0],
    ["replace_phone", ok, {}, 11],
    ["replace_phone", ok, {}, 2.5],
    ["replace_phone", ok, {}, "1"],
  ];
  const s = setup();
  try {
