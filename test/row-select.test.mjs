...
import { rowSelect } from "../app/renderer/src/components/row-select.ts";

/** 假元素：closest 只在选择器里含 hit 时命中（模拟按钮 / 输入框 / 勾选框 / 标记过的单元格） */
/** @param {string | null} [hit] */
function el(hit = null) {
  return { nodeType: 1, closest: (selector) => (hit && selector.includes(hit) ? { hit } : null) };
}
...
test("点交互元素不切换选中：按钮 / 输入框 / 勾选框 / 标记过的单元格", () => {
  for (const hit of ["button", "a", "input", "textarea", "select", "label", "[role='button']", "[contenteditable='true']", "[data-no-row-select]"]) {
    const s = setup({ keys: [] });
    s.click({ id: "a" }, el(hit));
