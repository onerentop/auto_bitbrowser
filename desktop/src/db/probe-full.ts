...
import { AccountRefreshRepository } from "./account-refresh-repository.ts";
...
const ar = new AccountRefreshRepository(db);

const out = {
  comprehensive: io.getComprehensiveAccountData(),
  pool: re.getPool(),
  bindings: re.getAllBindings(),
  recentTasks: ar.getRecentTasks(5),
};
db.close();
fs.writeFileSync(process.argv[2] as string, JSON.stringify(out), "utf8");
console.log(`comprehensive=${out.comprehensive.length} pool=${out.pool.length} bindings=${Object.keys(out.bindings).length} tasks=${out.recentTasks.length}`);