...
5. "Error" 或 "错误"`,
  ...
  // 真机（2026-09-24）：设备页是**中文**界面，observe 返回的描述里没有 "device" 这种英文词。
  // 原来「按 observe 描述文本过滤 device」的取列表方式在真机上恒为空 → 任务永远报「未找到其他设备」
  // 成功，实际一个设备都没踢（假成功）。改用页面内按结构读会话条目（li.K6ZZTd）+ 坐标点击 +
  // 点「退出账号」+ 用「还没退出的会话是否清零」核对，这几条提示词因此被替换或删除，
  // 不再与 Python 逐字一致（见 src/engine/operations/kick-devices.ts 头注释）。
  "kick_devices.py": [
