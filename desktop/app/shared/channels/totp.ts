 ...
 *
 * 对标 gui/import_totp_interface.py。
 */
// 仅类型导入：ipc.ts 反向 import 本文件，type-only 不会形成运行时循环
import type { TaskInfo } from "../ipc.ts";

export const TOTP_INVOKE = {
  /** 二维码文本 → TOTP 条目（对标 _processImages :652 里 extract_totp_secrets_from_image 拿到文本之后的部分） */
  totpParseUris: "abb/totp/parseUris",
  /** 文本模式解析（对标 _parseTextInput :552-619） */
  totpParseText: "abb/totp/parseText",
  /** 与数据库账号匹配（对标 _matchWithDatabase :708-751） */
  totpMatch: "abb/totp/match",
  /** 导入选中条目（后台任务，对标 ImportWorker :56-196） */
...
/**
 * 解析出的一条 TOTP 记录（对标 OTPAccount / TextOTPAccount 在界面上用到的字段）。
...
/** parseUris 的单张图片结果（用于逐图写日志，对标 :667-679） */
...
/** 文本模式的一行错误（对标 error_lines 的 (i, line, reason)） */
...
export interface TotpParseTextResult {
  ...
  /** 解析过程日志（对标 :564-609 的 self.log，已去掉 Python 里用于空行的 "\n" 前缀） */
  logs: string[];
}

/** 匹配状态（对标 STATUS_TEXT :203-207） */
...
export interface TotpMatchRow {
  ...
  /**
   * 当前密钥展示值（对标 :807-817）：前 8 位，超过 8 位追加 "..."；
 ...
 * password 只在 kind === "text" 时生效（对标 :130 isinstance(otp_acc, TextOTPAccount)）。
 */
...
/** 导入结果（对标 finished_import 信号 :61 / :196，字段名沿用 Python 局部变量名） */
export interface TotpImportResult {
  ...
  /** 因停止而未处理的条目数（Python 无停止功能，恒为 0 时即与 Python 一致） */
