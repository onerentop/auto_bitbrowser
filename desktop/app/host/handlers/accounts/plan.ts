 ...
 *
 * 每个分支逐条对标 gui/account_manager_interface.py 里对应按钮的处理函数（行号见各 case）。
 * Python 在界面层边判断边弹框；这里把判断结果整理成一份计划：
...
export async function planAction(action: AccountsAction, rows: readonly SelectedRow[], env: PlanEnv): Promise<Plan> {
  // 界面表格里的 (email, browser_id)，对标 _getSelectedRows（:737）
  const pairs = rows.map((r) => [r.email, r.browserId] as const);
  const selected = () => resolveSelectedAccounts(env.repo, pairs);
  // 与 Python 的有意偏差：worker 类任务的窗口 ID 一律以数据库的 browser_profile_id 为准，
  ...
  // 对标 _checkTaskConflicts：后端只允许一个任务，所以所有 flag 归并为 workerRunning
  ...
  switch (action) {
    ...
    case "delete_with_windows": {
      ...
      // 与 Python 的有意偏差：窗口 ID 一律取数据库里的 browser_profile_id，而不是界面行上的旧值。
      // 行上的值与数据库不一致（界面数据过期）时整条跳过 —— 既不删账号也不删窗口，只记日志，
      // 避免误删已被重新绑定给其他账号的窗口。Python 直接使用表格中的窗口 ID。
    ...
    // ---------- 右键「删除账号和窗口」（:1649-1685） ----------
    // 与 Python _deleteAccountWithWindow 的差异：
    //   1. 执行方式：Python 在界面线程同步执行；这里与批量删除共用后台任务（batch_delete），
    //      结束后由任务完成事件刷新列表、弹「删除完成」提示。
    //   2. 冲突检查：Python 右键删除不检查运行中的任务；这里与批量删除一样走 _checkTaskConflicts（等待后再删除）。
    //   3. 顺序与日志：Python 先关闭 / 删除窗口再删账号，日志为「已关闭窗口 / 已删除窗口 / 删除窗口失败 / 已删除账号」；
    //      这里走 executeBatchDelete，先删账号成功后再删窗口，日志为「开始批量删除…」「已删除: email」
    //      「批量删除完成: …」「已删除 N 个窗口」。
