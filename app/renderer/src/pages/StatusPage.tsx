...
const HOST_STATE_VIEW: Record<HostState, { tone: Tone; text: string }> = {
  ready: { tone: "ok", text: "就绪" },
  starting: { tone: "warn", text: "启动中" },
  crashed: { tone: "bad", text: "已崩溃" },
  stopped: { tone: "idle", text: "已停止" },
};
...
/** 圆点 + 文字 */
function StatusDot({ tone, children }: { tone: Tone; children: ReactNode }): ReactElement {
  const t = useTokens();
  return (
    <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}>
      <span aria-hidden style={{ width: 8, height: 8, borderRadius: "50%", background: toneColor(t, tone), flex: "none" }} />
      <span style={{ color: tone === "idle" ? t.muted : toneColor(t, tone), fontWeight: 500 }}>{children}</span>
    </span>
  );
}
...
export function StatusPage(): ReactElement {
  ...
  const ixView: { tone: Tone; text: string } = !hostReady
    ? { tone: "idle", text: "未检测" }
    : ix.data?.reachable
      ? { tone: "ok", text: "已连接" }
      : ix.data || ix.error
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <PageHeader title="运行状态" description="查看后端进程、ixBrowser 本地服务与运行时版本；后端异常时可在这里重启。" />
      <Panel>
        <Section
          first
          title="后端进程"
        ...
        >
          <Descriptions {...DESC_PROPS} column={{ xs: 1, md: 2 }}>
            <Descriptions.Item label="状态">
              <Space size={8} wrap>
                {stateView ? <StatusDot tone={stateView.tone}>{stateView.text}</StatusDot> : <StatusDot tone="idle">未知</StatusDot>}
        ...
        >
          ...
          <Descriptions {...DESC_PROPS} column={{ xs: 1, md: 2 }}>
            <Descriptions.Item label="状态">
              <StatusDot tone={ixView.tone}>{ixView.text}</StatusDot>
            </Descriptions.Item>
